...
#include "net/http/http_response_parser.h"
#include "poller.h"
			...
			if (!ok) {
				...
				while (true) {
					...
					if (n > 0) {
						...
						if (!response_parser.parse(std::string_view(buf, static_cast<std::size_t>(n)))) {
							LOG_ERROR("Failed to parse upstream response on fd {}", upstream_fd);
							ok = false;
							final_errno = EPROTO;
							break;
						}
						if (response_parser.is_message_complete()) {
							break;
						}
						continue;
					}
					...
					if (errno == EAGAIN || errno == EWOULDBLOCK) {
						if (!wait_fd(upstream_fd, EPOLLIN)) {
							if (errno == ETIMEDOUT &&
								out_response.size() > 0 &&
								!response_parser.is_message_complete() &&
...
bool http_proxy::forward_connect_tunnel(int client_fd, const http_conn::request_info& req,
	...
	const int upstream_fd = upstream.get_fd();
	const std::string connect_established_response =
		http_message_builder::build_connect_established_response("hpserver");

	LOG_DEBUG("CONNECT tunnel: sending 200 Connection Established to client fd {}", client_fd);
	if (!send_all_nonblocking(client_fd, connect_established_response)) {
