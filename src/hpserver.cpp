...
int hpserver::start_proxy_session(int client_fd) {
    ...
    if (upstream_connector::open_nonblocking(req.host, req.port, session->upstream_fd,
...
int hpserver::handle_client(int client_fd, std::uint32_t events_mask) {
    ...
    if (proxy_event_dispatcher::handle_client_session_io(session, events_mask) != 0) {
        return -1;
    }
...
int hpserver::handle_upstream(int upstream_fd, std::uint32_t events_mask) {
    ...
    if (proxy_event_dispatcher::handle_upstream_session_io(session, events_mask) != 0) {
        return -1;
    }
